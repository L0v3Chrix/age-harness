...
def display_command_name() -> str:
    """Return the customer-facing command name for this invocation."""
    stem = os.path.basename(sys.argv[0]).lower()
    return "age" if stem in {"age", "age.exe", "age.cmd"} else "hermes"
...
def build_top_level_parser():
    ...
    """
    prog = display_command_name()
    brand = "AGE by Genesis Labs" if prog == "age" else "Hermes Agent"
    parser = argparse.ArgumentParser(
        prog=prog,
        description=f"{brand} - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=_build_epilogue(prog),
    )
    ...
    chat_parser = subparsers.add_parser(
        ...
        description=f"Start an interactive chat session with {brand}",
    )
