"""Tests for ``age dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``age update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No age dashboard processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "2 age/hermes dashboard process(es) running" in out
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``age dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``age dashboard --stop`` / ``--status`` actually parse."""
